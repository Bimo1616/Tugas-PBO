...
def konversi_suhu():
    try:
        ...
        # Update label_hasil with a more attractive font style
        result_text = f"Hasil konversi: {suhu_celsius:.2f} Celsius, {suhu_fahrenheit:.2f} Fahrenheit, {suhu_kelvin:.2f} Kelvin"
        label_hasil.config(text=result_text, bg='lightgreen', font=('Helvetica', 14, 'bold italic'))
    except ValueError:
        label_hasil.config(text="Masukkan angka yang valid untuk suhu.", bg='salmon', font=('Helvetica', 12, 'italic'))
...
# Membuat label
label_suhu = tk.Label(root, text="Masukkan suhu:", font=('Helvetica', 12, 'bold'))
label_suhu.grid(row=0, column=0, padx=10, pady=10)

# Membuat entry (input) untuk suhu
entry_suhu = tk.Entry(root, font=('Helvetica', 12))
...
tombol_konversi = tk.Button(root, text="Konversi", command=konversi_suhu, font=('Helvetica', 12, 'bold'))
tombol_konversi.grid(row=1, column=0, columnspan=3, pady=10)

# Membuat label hasil konversi
label_hasil = tk.Label(root, text="Hasil konversi: ", font=('Helvetica', 12, 'italic'))
